...
const CanvasWrapper: FC<{ children: ReactNode }> = ({ children }) => (
  <Canvas dpr={[1, 2]} frameloop="always" className="w-full h-full relative" style={{ background: 'transparent' }} gl={{ alpha: true }}>
    {children}
  </Canvas>
);
...
const Beams: FC<BeamsProps> = ({
  ...
  vec3 getNormal(vec3 pos) {
    ...
    gl_FragColor.rgb -= randomNoise / 15. * uNoiseIntensity;`
        ...
        material: { fog: true, transparent: true, opacity: 0.5 },
        uniforms: {
      ...
      <CanvasWrapper>
        <group rotation={[0, 0, degToRad(rotation)]}>
          ...
          {[-20, -15, -10, -5, 0, 5, 10, 15, 20].map((x) => (
            <DirLight key={x} color={lightColor} position={[x, 3, 10]} />
          ))}
        </group>
